package javaSE.javaseIntermediate.javase1;
 ...
 *
 * 错误
 * 错误比异常更严重 异常就是不同寻常 但不一定会导致致命的问题 而错误是致命问题 一般出现错误可能JVM就无法继续正常运行了 比如OutOfMemoryError就是内内存溢出错误(内存占用已经超出限制 无法继续申请内存了)
 *                  int[] arr = new int[Integer.MAX_VALUE]; // 能创建如此之大的数组吗
 *
 * 运行后得到以下内容:
 *                  Exception in thread "main" java.lang.OutOfMemoryError: Requested array size exceeds VM limit
 *
 * 错误都继承自Error类 一般情况下 程序中只能处理异常 错误是很难进行处理的 Error和Exception都继承自Throwable类 当程序中出现错误或异常时又没有进行处理时 程序(当前线程)将终止运行:
